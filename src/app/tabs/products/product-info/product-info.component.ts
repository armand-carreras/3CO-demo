...
import { DemoGuardService } from 'src/app/shared/services/demo-guard.service';
...
export class ProductInfoComponent implements OnInit, ViewWillLeave, ViewWillEnter, OnDestroy {
  ...
  constructor(
    private demoGuard: DemoGuardService,
  ...
  ngOnDestroy(): void {
    console.log('Destroying review page component: setting everything to 0');
    this.cleanup();
  }
  ...
  private cleanup(): void {
    ...
    this.loadingReviewTranslations.clear();
  }
  ...
  public async translateDescription() {
    ...
    try {
      ...
      this.demoGuard.showDemoWarning();
  ...
  public async translateReview(review: Review) {
    ...
    try {
      ...
      // Perform translations for both title and description (this will cache them in the service)
      //const translationPromises = [];

      if (review.title) {
        //translationPromises.push(this.anylangService.translate(review.title, targetLang));
        this.demoGuard.showDemoWarning();
      }

      if (review.description) {
        //translationPromises.push(this.anylangService.translate(review.description, targetLang));
        this.demoGuard.showDemoWarning();
      }

      //await Promise.all(translationPromises);

      // Now enable the pipe to display the cached translations
