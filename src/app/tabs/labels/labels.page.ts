...
export class LabelsPage implements OnInit, OnDestroy, ViewWillEnter {
  ...
  ngOnInit() {
    ...
    this.setRandomLabel();
    // Subscribe to DB readiness to update featured label when data arrives
    this.dbReadySubscription = this.labelService.allState().subscribe((isReady) => {
      if (isReady) {
        this.setRandomLabel();
      }
    });
  ...
  public async scan() {
    await this.demoGuard.showDemoWarning()
 ...
 /*  public showMoreLabelDetectedInfo(label: Label) {
  ...
  private setRandomLabel() {
    const label = this.labelService.featuredLabel?.[0];
    if (label) {
      this.randomLabel = {
        ...label,
        logo: label.logo ?? '/assets/databases/No_Image_Available.jpg'
      };
    }
  }
