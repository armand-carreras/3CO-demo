...
export class LabelSQLiteHandlerService {
  ...
  async loadAll() {
    if (!this.db) {
      console.warn('loadAll: Database not initialized');
      return;
    }
    try {
      ...
      console.warn('loadAll: labelsBase64 table not available:', e);
    }
  }
  ...
  async getImageById(id: number) {
    ...
    public async getRandomLabel() {
      if (!this.db) {
        console.warn('getRandomLabel: Database not initialized');
        return;
      }
      try {
        ...
        console.warn('getRandomLabel: labelsBase64 table not available:', e);
      }
    }
