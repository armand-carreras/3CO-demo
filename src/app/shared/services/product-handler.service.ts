...
export class ProductHandlerService {
  ...
  public async loadProducts(page: number = 1, searchQuery: string = '', categories: string[] = []) {
    console.log('Service loadProducts called for page:', page, 'query:', searchQuery, 'categories:', categories);
    try {
      const response = await this.fetchProducts(page, 10, searchQuery, categories);
      if (page === 1) {
        this.productList.next(response.products);
      } else {
        const currentProducts = this.productList.getValue();
        console.log('Appending products. Current count:', currentProducts.length, 'New products:', response.products.length);
        this.productList.next([...currentProducts, ...response.products]);
      }
      return response;
    } catch (error) {
   ...
   */
  private async fetchProducts(page: number = 1, per_page: number = 10, searchQuery: string = '', categories: string[] = []) {
    ...
    const response = await firstValueFrom(
      this.http.get<productGET>(`${environment.paths.base_api}${environment.paths.products}`, { params }).pipe(
        tap((res) => {
          console.log(res);
        }),
